...
class TestPlaceOrderMissingItemId:
    ...
    def test_when_item_id_missing_then_returns_400_and_validation_error(self):
        # Given: an authenticated festival goer but an item without `id`
        req = CreerCommandeRequest("festivalier-1", [{"quantite": 1}])

        # When
        response = self.client.creer_commande(req)

        # Then: API should return 400 Bad Request with a validation error about missing item_id
        assert response.status_code == 400
        body = response.json()
        assert "errors" in body, f"expected validation errors in response, got: {body}"
        assert any(err.get("field") == "item_id" for err in body.get("errors", []))
